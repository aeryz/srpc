...
fn main() {
    ...
    let mut codec: SimpleCodec<Data> = SimpleCodec::new();
    let mut data: Vec<u8> = Vec::new();
    for _ in 0..20 {
        let a = Data { a: rng.gen() };
        let bytes = serde_json::to_vec(&a).unwrap();
        data.extend(&bytes.len().to_le_bytes());
        data.extend(bytes.as_slice());
    }
    codec.extend(data.as_slice());

    while let Some(codec) = codec.drain() {
        println!("{:?}", codec);
    }
}
